...
from .serializer import UserSerializer, CreateUserSerializer, LoginUserSerializer, IdUserSerializer, PlaceSerializer, CreatePlaceSerializer, SearchPlaceSerializer, DeletePlaceSerializer, CreateTransactionSerializer
...
class CreateTransaction(generics.ListCreateAPIView):
    ...
    def get(self, request, id_client, id_place, initialDate, finalDate, *args, **kwargs):      #initialDate and finalDate is datatime
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
        ...
        if serializer.is_valid():
            ...
            payment =int(delta_time.days) * price
...
class Balance(generics.ListCreateAPIView):
    ...
    def setBalance(self, id, balance):
        search = User.objects.get(pk = id)
        search.balance = float(balance)

    

class SchedulerBalance(generics.ListCreateAPIView):
    countTask = 0
    id = []
    balance = []
    date = []

    def get(self, request, id, balance, dia, mes, ano, *args, **kwargs):
        self.countTask += 1
        self.id.append(id)
        self.balance.append(balance)
        self.date.append(datetime(ano, mes, dia))
        self.my_background_task()
        return Response(status=status.HTTP_102_PROCESSING)

    
    @background(schedule=10)  # A tarefa será verificada a cada 10 minutos
    def my_background_task(self):
    # Aqui você pode colocar a lógica da função que deseja executar
    # quando a data especificada for alcançada.
        for i in range(self.countTask):
            if datetime.now().date() >= self.date[i]:
                Balance.setBalance(self.id[i], self.balance[i])
                data_response = {
                    "balance" : self.balance[i]
                }
                return JsonResponse(data_response)
